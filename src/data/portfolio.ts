...
export const WORKS: Work[] = [
  ...
  },
  {
    ...
    desc: '公司 AI 助手越做越多（经营、采购、录单…），每个都重复造轮子。做一个统一的多租户平台，把模型接入、知识库、记忆、对话通道这些公共能力沉下去，让上层助手专心做业务。首个产品「经营顾问」就建在这套地基上：老板想看经营情况常要等人导数据做表，又慢又滞后，靠知识库与记忆能力做成「会看数、会分析、会提建议」的助手——单轮问答、多步诊断、自动经营日报三层用法。',
    metrics: [
      ['架构演进', '前后 15 版'],
      ['模型网关', '3 档路由'],
      ['通道', 'Web + 微信'],
    ...
    achievements: [
      ...
      '知识库方案：系统知识库与租户知识库自动融合，配生鲜行业知识词表；全生命周期切片 → 向量化 → 提炼结构化知识 → 人工审核上线，冲突自动拦下',
      '记忆系统：从对话记忆到长期画像分层管理，引导式记忆按缺什么问什么（门店数/主营品类），回答越个性化，靠降频与缓存把 Token 成本控住',
      '多租户隔离 + Web/微信双通道，每个用户一个 1:1 私人助手；落地首个预设助手「经营顾问」，跑通单轮问答→多步诊断→经营日报三层用法',
    ],
    media: {
      type: 'arch',
