...
window.PORTFOLIO = {
  ...
  works: [
    ...
    },
    {
      ...
      desc: "公司 AI 助手越做越多（经营、采购、录单…），每个都重复造轮子。做一个统一的多租户平台，把模型接入、知识库、记忆、对话通道这些公共能力沉下去，让上层助手专心做业务——目标不只是省重复建设的成本，更是把知识库越用越准地沉淀成生鲜供应链行业最垂直的「知识飞轮」，同时让这套底座成为公司后续每一个新 Agent 的统一可视化研发地基。",
      metrics: [["架构演进", "前后 15 版"], ["模型网关", "3 档路由"], ["通道", "Web + 微信"], ["隔离", "多租户"]],
      achievements: [
        "从 0 规划整个平台，架构方案前后改了 15 版，定下整体思路与工程骨架",
        "多模型网关：按「便宜快 / 均衡 / 高智能」分档路由，换底层模型只改配置",
        "知识库方案：系统知识库与租户知识库自动融合，配生鲜行业知识词表——定位是行业知识飞轮，越用越准，不是一次性问答",
