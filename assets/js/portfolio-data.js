...
window.PORTFOLIO = {
  ...
  works: [
    ...
    },
    {
      ...
      title: "智能牙刷 · 交互式 3D 产品页",
      subtitle: "自主设计 + 手写 Three.js WebGL · 全代码渲染 · 2026",
      tags: ["aesthetic", "fullstack", "viz"],
      desc: "一个可交互的智能牙刷 3D 产品页。从机身、智能屏到刷毛、金属反射全部用 Three.js 程序化建模渲染，没有一张实拍图；首屏产品穿插于文字（3D 透视层叠），滚动左右交替、平滑丝滑，逐层拆解后可点击零件看说明，双色实时切换材质——纯前端、可离线，用来证明「会写代码的 PM」的网页美学与特效落地能力。",
      metrics: [["渲染", "Three.js WebGL"], ["建模", "全代码·含智能屏"], ["交互", "点击零件 / 滚动拆解"], ["材质", "双色实时切换"]],
      achievements: [
        "全代码渲染：LatheGeometry 旋出阳极氧化铝机身、InstancedMesh 种植刷毛、机身发光 OLED 智能屏（模式/电量/计时），Canvas→PMREM 卷积出真实金属反射，ACES 色调映射",
        "首屏 3D 透视：产品穿插于背景大字（z 层叠），文字让位不挡产品；滚动各屏左右交替（zig-zag），平滑 scrollProgress 让镜头/构图/拆解丝滑不卡顿",
        "拆解可交互：滚动沿轴「伸缩拆解」为刷头 → 精控动力马达 → 电池；Raycaster 点击任意零件弹出说明卡并跟随锚点",
        "细节：太空灰/冰莓粉材质实时着色、四大方案 hover 联动刷头高亮、数字滚动、环境浮尘、reduced-motion 降级、WebGL 兜底页",
        "零外网依赖：Three.js 本地打包进 demo，离线可跑；「立即选购」直达电商店铺"
      ],
      media: { type: "iframe", src: "./usmile/", url: "智能牙刷 · 实时 3D（拖拽 / 滚动 / 点击零件）", caption: "浏览器直跑的实时 3D 产品页（全代码渲染，无实拍图）。抽屉内可交互，完整体验建议全屏打开。" },
      links: [
        { label: "全屏打开 3D 产品页 →", href: "./usmile/", primary: true }
      ],
      note: "通用智能牙刷产品概念页，非任何品牌官方页面；视觉与 3D 全部自主编码，无实拍图或官方素材，产品信息为通用行业说法。"
    },
    {
      ...
      desc: "公司 AI 助手越做越多（经营、采购、录单…），每个都重复造轮子。做一个统一的多租户平台，把模型接入、知识库、记忆、对话通道这些公共能力沉下去，让上层助手专心做业务——目标不只是省重复建设的成本，更是把知识库越用越准地沉淀成生鲜供应链行业最垂直的「知识飞轮」，同时让这套底座成为公司后续每一个新 Agent 的统一可视化研发地基。首个产品「经营顾问」就建在这套地基上：老板想看经营情况常要等人导数据做表，又慢又滞后，靠知识库与记忆能力做成「会看数、会分析、会提建议」的助手——单轮问答、多步诊断、自动经营日报三层用法，把知识飞轮的价值直接体现在老板的日常经营决策里。",
      metrics: [["架构演进", "前后 15 版"], ["模型网关", "3 档路由"], ["通道", "Web + 微信"], ["隔离", "多租户"]],
      achievements: [
        "从 0 规划整个平台，架构方案前后改了 15 版，定下整体思路与工程骨架",
        "多模型网关：按「便宜快 / 均衡 / 高智能」分档路由，换底层模型只改配置",
        "知识库方案：系统知识库与租户知识库自动融合，配生鲜行业知识词表——定位是行业知识飞轮，越用越准，不是一次性问答；全生命周期切片 → 向量化 → 提炼结构化知识 → 人工审核上线，冲突自动拦下",
        "记忆系统：从对话记忆到长期画像分层管理，引导式记忆按缺什么问什么（门店数/主营品类），回答越个性化，靠降频与缓存把 Token 成本控住",
        "多租户隔离 + Web/微信双通道，每个用户一个 1:1 私人助手；落地首个预设助手「经营顾问」，跑通单轮问答→多步诊断→经营日报三层用法，验证知识飞轮直接服务经营决策",
        "统一 Agent 承载底座：模型网关/知识库/记忆/多租户/通道能力全部下沉平台层，后续新 Agent 接入即复用，不用重新造轮子，同时是团队新 Agent 研发的可视化工程地基"
      ],
